...
def test_code_doctor_syntax_and_semantic_repair(client, developer_headers):
    """Code Doctor endpoint (POST /api/ai/fix-code) must diagnose broken code and produce direct corrected syntax."""
    payload = {
        "code": "print('hello world",
        "language": "python",
    }
    response = client.post("/api/ai/fix-code", json=payload, headers=developer_headers)
    assert response.status_code == 200
    data = response.json()
    assert data["is_correct"] is False
    assert "corrected_code" in data
    assert "print('hello world')" in data["corrected_code"] or 'print("hello world")' in data["corrected_code"]
    assert "root_cause" in data
    assert "prevention_tip" in data


def test_code_doctor_valid_code_confirmed(client, developer_headers):
    """Code Doctor should recognize already correct code with is_correct: True."""
    payload = {
        "code": "def add(a, b):\n    return a + b",
        "language": "python",
    }
    ...
    assert data["ast_verified"] is True
    assert data["generated_unit_tests"] is not None


def test_code_doctor_security_vulnerability_cwe89_audit(client, developer_headers):
    """Code Doctor identifies SQL injection (CWE-89) and patches it with parameterized query binding."""
    payload = {
        "code": "cursor.execute(f'SELECT id, username FROM users WHERE id = {user_id}')",
        "language": "python",
        "audit_profile": "security"
    }
    response = client.post("/api/ai/fix-code", json=payload, headers=developer_headers)
    assert response.status_code == 200
    data = response.json()
    ...
    assert data["security_findings"][0]["cwe_id"] == "CWE-89"
    assert "SQL Injection" in data["security_findings"][0]["title"]
    assert "%s" in data["corrected_code"] or ":id" in data["corrected_code"] or "params" in data["corrected_code"]


def test_code_doctor_big_o_complexity_optimization(client, developer_headers):
    """Code Doctor detects quadratic O(N²) nested list membership scan and optimizes to linear O(N) Hash Set."""
    payload = {
        "code": "def find_common(list_a, list_b):\n    res = []\n    for item in list_a:\n        if item in list_b:\n            res.append(item)\n    return res",
        "language": "python",
        "audit_profile": "performance"
    }
    response = client.post("/api/ai/fix-code", json=payload, headers=developer_headers)
    assert response.status_code == 200
    data = response.json()
    assert data["complexity_analysis"] is not None
    assert data["complexity_analysis"]["time_complexity_original"] == "O(N²)"
    assert data["complexity_analysis"]["time_complexity_optimized"] == "O(N)"
    assert "set" in data["corrected_code"]


def test_code_doctor_concurrency_race_condition(client, developer_headers):
    """Code Doctor detects uncoordinated shared mutable state mutation under async and wraps in Mutex Lock."""
    payload = {
        "code": "active_users = 0\n\nasync def increment_user():\n    global active_users\n    active_users += 1\n    return active_users",
        "language": "python",
        "audit_profile": "concurrency"
    }
    response = client.post("/api/ai/fix-code", json=payload, headers=developer_headers)
    assert response.status_code == 200
    data = response.json()
    assert len(data["concurrency_risks"]) >= 1 or "Lock" in data["corrected_code"]


def test_code_doctor_resource_safety_context_manager(client, developer_headers):
    """Code Doctor identifies unclosed file descriptors and wraps in Python context manager (with open)."""
    payload = {
        "code": "f = open('data.json', 'r')\ndata = f.read()\nreturn data",
        "language": "python",
        "audit_profile": "resource_safety"
    }
    response = client.post("/api/ai/fix-code", json=payload, headers=developer_headers)
    assert response.status_code == 200
    data = response.json()
    assert "with open" in data["corrected_code"]
    assert len(data["diff_lines"]) >= 1
